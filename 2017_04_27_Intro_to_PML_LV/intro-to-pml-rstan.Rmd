...
data {
  ...
  int<lower=1> N1;       // sample size of training set
  matrix[N1, D] x1;      // obs of training set
  vector[N1] y1;         // response of training set

  int<lower=1> N2;       // sample size of test set
  matrix[N2, D] x2;     // obs of test set
}
parameters {
  real intercept;
  vector[D] slope;
...
model {
  ...
  y1 ~ normal(x1 * slope + intercept, sigma);
}
generated quantities {
  vector[N2] y2;
  for (n in 1:N2)
    y2[n] = normal_rng(x2[n] * slope + intercept, sigma);
}
"
```
